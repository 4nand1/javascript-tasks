...
addBtn.addEventListener("click", () => {
  if (input.value.trim() === "") return;
  content.push({ id: id++, text: input.value, isDone: false });
  input.value = "";
  render();
});
...
buttons.forEach((btn, i) => {
  btn.addEventListener("click", () => {
    ...
    btn.classList.add("chosen");
    type = i === 0 ? "All" : i === 1 ? "Active" : "Completed";
    render();
  });
});
...
const render = () => {
  const elements = content
    .filter((item) =>
      type === "All" ? true : type === "Active" ? !item.isDone : item.isDone
    )
    .map((item) => ListItem(item, content.indexOf(item)))
    .join("");
...
const addListeners = () => {
  ...
  document.querySelectorAll(".checkbox").forEach((checkbox) => {
    checkbox.addEventListener("change", (e) => {
      const index = Number(e.target.parentElement.dataset.index);
      content[index].isDone = e.target.checked;
      render();
    });
  });
};
